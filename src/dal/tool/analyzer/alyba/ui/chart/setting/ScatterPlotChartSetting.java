...
public class ScatterPlotChartSetting extends ChartSetting {
	...
	protected void createContents() {
		...
		FormData fd_lb_shape_size = new FormData();
		fd_lb_shape_size.left = new FormAttachment(grp_setting, 0, SWT.LEFT);
		fd_lb_shape_size.top = new FormAttachment(grp_setting, 0);
		lb_shape_size = new Label(grp_setting, SWT.CENTER);
		lb_shape_size.setLayoutData(fd_lb_shape_size);
		lb_shape_size.setFont(Utility.getFont());
		...
		chk_reg_linear.setFont(Utility.getFont());
	...
	public void reset(Chart chart) {
		if(chart != null) {
			if(chart instanceof ScatterPlotChart) {
				...
				scale_shape_size.setSelection(spChart.getShapeSizeNumber());
				...
				scale_shape_size.setSelection(distChart.getShapeSizeNumber());
				chk_reg_linear.setSelection(distChart.getShowRegressionLine());
	...
	public void configure(Chart chart) {
		if(chart instanceof DistributionChart) {
			...
			dist_chart.setShapeSizeNumber(scale_shape_size.getSelection());
			dist_chart.setShowRegressionLine(chk_reg_linear.getSelection());
			dist_chart.setShowRegressionEquation(chk_reg_equation.getSelection());
		} else {
			ScatterPlotChart sp_chart = (ScatterPlotChart)chart;
			sp_chart.setShapeSizeNumber(scale_shape_size.getSelection());
